...
import {
	processWebhookRetry,
	type WebhookRetryMessage,
} from "./src/lib/email/webhooks";
import {
	resolveIncomingMail,
	forwardMessage,
} from "./src/lib/email/incoming";
...
export default {
	async fetch(
		...
		if (url.pathname === "/api/realtime") {
			...
			return hub.fetch(
				new Request("https://mailflare-realtime/connect", request),
			);
		}
	...
	async email(
		...
		try {
			...
			const decision = await resolveIncomingMail(
				env,
				message.from,
				message.to,
			);
			...
			if (decision?.action === "reject") {
				message.setReject(
					decision.rejectReason ??
						"Message rejected by routing rule",
				);
				return;
			}
			...
			if (
				message.headers.get(MAILFLARE_FORWARDED_HEADER) !== "1"
			) {
				const forwardingDestination =
					await getAccountForwardingDestination(
						env,
						message.to,
					);
			...
			// Store the raw email in D1-backed blob storage instead of R2.
			const rawBlobKey = await storeRawEmail(
				env,
				message.from,
				message.to,
				message.raw,
			...
			try {
				if (isInboundQueueMessage(msg.body)) {
					...
					await processWebhookRetry(
						env,
						msg.body as WebhookRetryMessage,
					);
				} else {
					await processOutboundQueue(
						env,
						msg.body as OutboundQueueMessage,
					);
				}
